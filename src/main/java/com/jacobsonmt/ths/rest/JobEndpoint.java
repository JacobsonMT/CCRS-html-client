package com.jacobsonmt.ths.rest;
...
public class JobEndpoint {
    ...
    public THSJob.THSJobVO getJob( @PathVariable String jobId) {
    ...
    public THSJob.THSJobVO getJob2( @RequestParam(value = "jobId") String jobId) {
        return createJobValueObject( jobManager.getSavedJob( jobId ) );
    }
    ...
    public String submitJob(@RequestParam(value = "label", required = false) String label,
        ...
        THSJob job = jobManager.createJob( label, label, pdbContent, fastaContent, proteinChainIds, email, hidden );
        String msg = jobManager.submit( job );
        msg = msg.isEmpty() ? "Submitted" : msg;
        log.info( "Job " + msg + ": " + job.getJobId() );
        return "Job " + msg + ": " + job.getJobId();

    }

    private THSJob.THSJobVO createJobValueObject( THSJob job) {
        if ( job == null ) {
            return null;
        }
