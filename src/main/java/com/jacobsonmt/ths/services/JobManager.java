...
import com.jacobsonmt.ths.model.THSJobResult;
import com.jacobsonmt.ths.model.PurgeOldJobs;
...
import static com.jacobsonmt.ths.model.THSJob.inputStreamToString;
...
public class JobManager {
    ...
    private final Set<THSJob> jobQueueMirror = new LinkedHashSet<>();

    // Secondary user queues or waiting lines. One specific to each user/session.
    private Map<String, Queue<THSJob>> userQueues = new ConcurrentHashMap<>();

    // Contains map of token to saved job for future viewing
    private final Map<String, THSJob> savedJobs = new ConcurrentHashMap<>();
                    ...
                    public FileVisitResult visitFile( Path path,
                        ...
                        if ( matcher.matches( path ) ) {
                            try ( ObjectInputStream ois = new ObjectInputStream( Files.newInputStream( path ) ) ) {
                                THSJob job = ( THSJob ) ois.readObject();
                                ...
                                THSJobResult result = new THSJobResult(
    ...
    public THSJob createJob( String userId,
                             String label,
                             ...
                             String email,
                             boolean hidden ) {
        THSJob.THSJobBuilder jobBuilder = THSJob.builder();
    ...
    private void submitToProcessQueue( THSJob job ) {
    ...
    private void submitToUserQueue( THSJob job ) {
        log.info( "Submitting job (" + job.getJobId() + ") for user: (" + job.getUserId() + ") to user queue" );

        Queue<THSJob> jobs = userQueues.computeIfAbsent( job.getUserId(), k -> new LinkedList<>() );
    ...
    private void submitJobFromUserQueue( String userId ) {
        ...
        if ( cnt < applicationSettings.getUserProcessLimit() ) {
            ...
            Queue<THSJob> jobs = userQueues.get( userId );
    ...
    public String submit( THSJob job ) {
    ...
    public THSJob getSavedJob( String jobId ) {
        THSJob job = savedJobs.get( jobId );
        if ( job !=null ) {
    ...
    private void saveJob( THSJob job ) {
    ...
    private boolean validateJob( THSJob job ) {
    ...
    public void onJobStart( THSJob job ) {
        ...
        synchronized ( jobQueueMirror ) {
            int idx = 1;
            for ( THSJob THSJob : jobQueueMirror ) {
                if ( !THSJob.isRunning() ) {
                    THSJob.setPosition( idx );
                    THSJob.setStatus( "Position: " + Integer.toString( idx ) );
                    idx++;
                }
            }
        }
    ...
    public void onJobComplete( THSJob job ) {
        job.setSaveExpiredDate( System.currentTimeMillis() + applicationSettings.getPurgeAfterHours() * 60 * 60 * 1000 );
    ...
    public List<THSJob.THSJobVO> listPublicJobs() {
        return Stream.concat(jobQueueMirror.stream(), savedJobs.values().stream())
                .distinct()
                .filter( j -> !j.isHidden() )
                .map( j -> j.toValueObject( true ) )
                .sorted(
                        Comparator.comparing( THSJob.THSJobVO::getPosition, Comparator.nullsLast(Integer::compareTo))
                                .thenComparing( THSJob.THSJobVO::getSubmittedDate, Comparator.nullsLast(Date::compareTo).reversed())
                                .thenComparing( THSJob.THSJobVO::getStatus, String::compareToIgnoreCase)
                )
